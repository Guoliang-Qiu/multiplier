...
Focused mutation: retain symmetric pruning of columns 0--4 and the column-5
compensation constant, but remove only the first ``a*b`` Booth row's ``k5``
partial-product term in column 5.  This is an asymmetric boundary-column
mutation: all other column-5 terms and the carry into column 6 remain live.
Exact constant folding, complement simplification, commutative hash-consing,
and common-data factoring of the top Booth mux reduce the resulting reachable
DAG without further functional approximation.  High signed columns and the
final carry-propagate adder are unchanged.
"""
...
PROD_CUT_A = 5
PROD_CUT_B = 5
OMIT_A_COL5 = frozenset({(0, "k5")})
OMIT_B_COL5 = frozenset()
COMP_COLS = (10, 11, 13, 15, 18)
ROUND_COLS: tuple[int, ...] = (5,)


def _gate(c: Circuit, kind: str, *inputs: int) -> int:
    """Apply exact Boolean identities, then share equal gate expressions."""
...
def _product_columns(
    ...
    zero: int, one_const: int, cut: int, omit_col5=frozenset(),
) -> list[list[int]]:
    ...
    for step in range(num_steps):
        ...
        def place(col: int, node: int, tag: str) -> None:
            if col < cut or (col == 5 and (step, tag) in omit_col5):
                return
        ...
        for k in range(3, q_bits_of_step):
            if two == zero:
                ...
                # The two clamped mux data inputs coincide in the top row.
...
def build_circuit():
    ...
    for left, right, cut, omit in (
        (a, b, PROD_CUT_A, OMIT_A_COL5),
        (x, d, PROD_CUT_B, OMIT_B_COL5),
    ):
        product = _product_columns(c, right, left, zero, one, cut, omit)
