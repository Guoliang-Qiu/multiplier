...
Focused local optimization: retain the parent's asymmetric column-5/column-6
partial-product pruning and column-6 compensation exactly, but canonicalize the
remaining DAG with exact constant propagation, idempotence, complement laws,
double-inverter elimination, commutative hash-consing, and an exact common-data
factorization in the top Booth mux.  This removes output-reachable gates without
changing any output truth table or approximation error.
"""
...
PROD_CUT_A = 5
PROD_CUT_B = 6
OMIT_A_COL5 = frozenset()
COMP_COLS = (10, 11, 13, 15, 18)
ROUND_COLS: tuple[int, ...] = (6,)


def _gate(c: Circuit, kind: str, *inputs: int) -> int:
    """Apply exact Boolean identities, then share identical gate expressions."""
    ...
    key = (kind, inputs)
    node = c._gate_cache.get(key)
    if node is None:
        node = c.add(kind, *inputs)
        c._gate_cache[key] = node
    return node
    ...
    for step in range(num_steps):
        ...
        for k in range(3, q_bits_of_step):
            if two == zero:
                ...
                # Both clamped mux data inputs are identical in the top row:
                # (one & x) | (two & x) == (one | two) & x.
