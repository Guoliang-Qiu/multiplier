"""Standalone qgl-style approximate signed 9-bit ``a*b + c*d`` circuit."""
...
def _product_columns(circuit, q_bits, m_bits, zero, one_const, cut):
    ...
    for step in range(groups):
        ...
        for k in range(WIDTH + 1):
            ...
            if two == zero or k == 0:
                ...
                p2 = _and(circuit, _bit(m_bits, k - 1, zero), two)
                magnitude = _or(circuit, p1, p2)
    ...
    for col in range(cut, OUT_WIDTH):
        if compensation >> col & 1:
            columns[col].append(one_const)
    return columns


def _finish(circuit, columns, zero):
...
def build_circuit() -> Circuit:
    ...
    columns = [[] for _ in range(OUT_WIDTH)]
    for x, y in ((a, b), (c, d)):
        product = _product_columns(circuit, y, x, zero, one, CUT)
        for col in range(OUT_WIDTH): columns[col].extend(product[col])
    circuit.output_ids = _finish(circuit, columns, zero)
    return circuit
