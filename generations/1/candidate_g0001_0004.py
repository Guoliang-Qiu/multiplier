...
the only bits below column 6 of each product are one weight-16 ``total``
bit at column 4 (shared top-bit rewrite) and four weight-32 bits at
column 5 (three special pp rewrites plus the shared carry): dropped mass
per product ``D = 16*n4 + 32*n5`` with ``n4 in [0,1]``, ``n5 in [0,4]``,
measured mean(D) ~= 68.5 raw units on the stock eval sample.  The mutation
removes that low region from the ``c*d`` product only (``PROD_CUT_B = 6``)
while the ``a*b`` product keeps its exact low columns, so the circuit
result stays on a fine 16-unit grid and round-to-nearest keeps working.
A single ONE leaf at column 6 (``ROUND_COLS = (6,)``, K = 64 ~= E[D])
recentres the residual to near-zero bias.  All low-column generation,
compression, and ripple-carry logic of one product disappears, filling the
Pareto gap between the sibling points (1062 gates / -53.82 dB, both
products cut) and (1174 gates / -57.62 dB, exact carry recovery).
Verified with the stock eval.py / gate_cost.py procedures:
gate_cost=1114, evm_db=-55.13, max_abs_error=1.75, bias=+0.096.
"""
...
# Statistical compensation for the sub-quantum mass removed from c*d:
# 64 raw units (one ONE leaf at column 6) ~= E[D] (~=68.5), recentring the
# residual error while keeping the result on the fine 16-unit grid.
