...
Local mutation: sub-quantum partial-product removal with a statistical
compensation constant (selective rounding).  The evaluator quantizes the
Q4.14 result to Q4.7, so every partial-product bit below column 7 is a
sub-quantum bit: those bits only matter through their sum
``D = 16*n4 + 32*n5 + 64*n6`` (at most 896 raw units, mean ~377, std ~125).
The parent reduced that whole region with the general compressor; the
sibling of this candidate recovered the region's carry *exactly* with a
dedicated tree.  This mutation instead deletes the entire sub-quantum
region from both products (``PROD_CUT = 7``) with no carry recovery at
all, and injects the constant 384 = 256 + 128 (``ROUND_COLS``) - the
dyadic value closest to the measured mean dropped mass - directly at the
truncation boundary.  The constant zeroes the error bias, leaving a
zero-mean residual of roughly one Q4.7 quantum rms, while the whole
low-column carry logic (about a fifth of the parent's weighted gate cost)
disappears.  Verified with the stock eval.py / gate_cost.py procedures.
"""
...
# Statistical compensation for the removed sub-quantum mass: 384 raw units
# (ONE at columns 7 and 8) ~= E[D] (~377), recentering the residual error.
ROUND_COLS: tuple[int, ...] = (7, 8)
