...
bias-matched compensation constant (selective rounding), combined with an
exact top-mux factoring (targeted subgraph substitution) in the Booth rows.

1) Truncation.  The evaluator quantizes the Q4.14 result to Q4.7 via
``floor(raw/128 + 0.5)``, so every partial-product bit below column 7 is a
sub-quantum bit.  In this generator the only bits below column 6 of a
product are one weight-16 ``total`` bit at column 4 (shared top-bit
rewrite) and four weight-32 bits at column 5 (three special pp rewrites
plus the shared carry): dropped mass ``D = 16*n4 + 32*n5`` with
``n4 in [0,1]``, ``n5 in [0,4]``, measured mean(D) ~= 68.5 raw units per
product on the stock eval sample.  The mutation removes that low region
from the ``c*d`` product only (``PROD_CUT_B = 6``) while the ``a*b``
product keeps its exact low columns, so the result stays on a fine 16-unit
grid and round-to-nearest keeps working.

2) Compensation.  ``ROUND_COLS = (3, 6)`` injects K = 72 = 64 + 8 - the
constant nearest the measured dropped mass plus the parent's structural
bias (68.5 + 7.5 ~= 76 raw units) - recentring the residual to near-zero
bias (measured +0.033, versus +0.096 for the K = 64 sibling).  The
column-3 ONE lands in a fully empty bucket and simply hardwires output
bit 3, so the extra 8 units of compensation cost no gates at all.

3) Exact top-mux factoring.  In every Booth row of steps 0..3 the most
significant partial-product bit (column 2*step+9) is built as
``NOT(OR(AND(one, x8), AND(two, x8)))`` because ``get_xor`` clamps both
mux terms to the same sign-bit node ``x8 = xors[8]``.  The mux therefore
factors exactly by distributivity: ``(one&x8) | (two&x8) == (one|two) & x8``.
(This also removes the 8 structurally duplicated ``AND(one, x8)`` gates the
clamp creates at rows k=8/k=9.)  For the surviving ``a*b`` product the
row-activity signal ``one|two`` is reused from the shared top-bit network
(step 0: ``pp_0_4 = q1|q0 == q0 | (~q0 & q1) = one|two``; step 1:
``pp_1_2``; step 2: the OR inside ``common``); the truncated ``c*d``
product and step 3 build one fresh OR per row.  The factoring saves 7
weighted gates on the exact product and 4 on the truncated one (11 total)
while leaving every output bit, and hence the Q4.7 ``evm_db``, unchanged.

The truncation sibling without factoring measured gate_cost=1114,
evm_db=-55.1334; this candidate measures gate_cost=1103 at the identical
evm_db=-55.1334, strictly dominating it.  Verified with the stock
...
# Bias-matched compensation for the removed sub-quantum mass of the c*d
# product: K = 72 = 64 + 8 ~= E[D] (68.5) plus the parent's structural bias
# (+7.5 raw units).  The column-3 ONE hardwires an empty output bit (free).
ROUND_COLS: tuple[int, ...] = (3, 6)
...
def _product_columns(c, q, m, zero, one_const, cut):
    ...
    for step in range((WIDTH + 1) // 2):
        ...
        # When the shared top-bit network survives the cut, steps 0-2 already
        # build an equivalent node; otherwise (and at step 3) it is built
        # lazily inside the top-mux branch.
        activity = None
        ...
        if cut <= 5:
            if step == 0:
                ...
                activity = pp_0_4  # q0|q1 == q0 | (~q0 & q1) == one | two
        ...
        for k in range(max(1, 6 - step * 2), q_width):
            ...
            if two == zero:
                ...
                # get_xor() clamps both top-bit mux terms to the same node
                # xors[8], so the mux factors exactly by distributivity:
                # (one & x8) | (two & x8) == (one | two) & x8.
                if activity is None: activity = _or(c, one, two)
                value = _and(c, activity, get_xor(k))
            else:
                value = _or(c, _and(c, one, get_xor(k)), _and(c, two, get_xor(k - 1)))
            if top: value = _not(c, value)
